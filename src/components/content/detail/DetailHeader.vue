<template>
  <div class="detail-header" v-if="detailHeader.artist">
    <img :src="detailHeader.picUrl" alt="" class="cover" />
    <div class="info">
      <h5>{{ detailHeader.name }}</h5>
      <div class="user">
        <img :src="detailHeader.artist.picUrl" alt="" class="profile" /><span>{{
          detailHeader.artist.name
        }}</span
        ><span>{{ formatTime(detailHeader.publishTime, 1000) }}</span>
      </div>
      <div class="tags" v-if="detailHeader.tags">
        标签：<span
