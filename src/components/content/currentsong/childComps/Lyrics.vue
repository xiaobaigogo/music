<template>
  <div class="lyrics" v-if="lyricsId">
    <h5>{{ lyricsName }}</h5>
    <div class="ul">
      <ul ref="lyricsScroll" v-if="lyricsContent.length">
        <li v-for="(lyricsRow, index) in lyricsContent" :key="index"
    ...
    watch: {
      ...
      lyricsId(newVal, oldVal) {
        ...
        getLyricsContent(newVal).then((res) => {
          ...
          res.lrc && this.formatLyrics(res.lrc.lyric);   
        });
      },
