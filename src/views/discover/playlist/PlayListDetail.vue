<template>
  <div class="play-list-detail">
    <div class="left">
      <playlist-detail-header :playListDetailHeader="playListDetailHeader"></playlist-detail-header>
      <button @click="playAll(tracksId)">播放全部</button>

      <list-content :trackLength="tracks.length" :playCount="playListDetailHeader.playCount" class="list-content">
        <template v-slot:table>
          <song-table :titles="['', '', '歌曲', '时长', '歌手', '专辑']" :contents="tracks"></song-table>
        </template>
      </list-content>
